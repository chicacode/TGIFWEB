...
function getStates(dataSenate) {
    ...
    var states = []; //create empty array for put 

    for (var i = 0; i < dataSenate.length; i++) {
        //cada estado del json es igual a state
        var state = dataSenate[i].state;
        //agrego en el array cada uno de los estados que encuentra
        states.push(state);
    ...
    //creo array vacio para ingresar cada state que no se repite
